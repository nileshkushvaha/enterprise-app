...
|---|---|---|
| Payments | An enabled provider has no key/secret; no webhook secret at all; no secret valid for the **wallet**, **booking**, or **package** webhook endpoint; the fake provider is on in production. (Warning) an endpoint verifies only through the legacy shared `razorpay_webhook_secret` line rather than its own `razorpay_{booking,package,wallet}_webhook_secret` field | Razorpay issues one secret per webhook. A missing wallet secret meant every recharge delivery was rejected with 401 (4 Sep 2026); a shared value that was right for one endpoint left six booking deliveries rejected with 401 while the payment sat captured (11 Sep 2026). Check the Razorpay dashboard's webhook delivery log for the endpoint's HTTP response code: 401 = wrong secret for that endpoint. |
