...
|---|---|---|
| Payments | An enabled provider has no key/secret; Stripe has no webhook secret; a required Razorpay endpoint (booking always; wallet and package while those features are on) has no webhook secret; the fake provider is on in production | Razorpay issues one secret per webhook endpoint. A missing wallet secret meant every recharge delivery was rejected with 401 (4 Sep 2026); a booking secret that was wrong for its endpoint left six booking deliveries rejected with 401 while the payment sat captured (11 Sep 2026). In the Razorpay dashboard's webhook delivery log, 401 = wrong secret for that endpoint. Fix path: Admin → Settings → Payment Gateway Settings → Razorpay. |
