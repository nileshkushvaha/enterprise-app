...
Meet recording requires the platform host present (2026-09-10). Google records only while the host or an in-org co-host is in the call, so a staff member joins each recorded class as the platform Workspace account. Spaces stay OPEN with auto-recording ON. Instructor Workspace accounts, Zoom, or an embedded classroom are the documented paths when staffing does not scale (docs/meetings.md §3).
