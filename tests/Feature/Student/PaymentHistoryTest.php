...
class PaymentHistoryTest extends TestCase
{
    ...
    public function test_pending_payment_on_an_active_booking_offers_to_complete_payment(): void
    {
        ...
        Livewire::actingAs($this->student)
            ...
            ->assertSeeHtml(route('dashboard.my-bookings.show', $booking));
    }
    ...
    /** The booking link opens that booking's own page, where the Pay now flow lives. */
    public function test_the_payment_link_opens_the_booking_detail_page(): void
    {
        $booking = $this->pendingBooking();
    ...
    /** The legacy `?booking=` deep link still lands on that booking's page. */
    public function test_legacy_deep_link_redirects_to_the_booking_detail_page(): void
    {
        $booking = $this->pendingBooking();

        $this->actingAs($this->student)
            ->get(route('dashboard.my-bookings', ['booking' => $booking->id]))
            ->assertRedirect(route('dashboard.my-bookings.show', $booking));
    }

    /** Another student's booking is never viewable, however it is reached. */
    public function test_another_students_booking_detail_page_is_forbidden(): void
    {
